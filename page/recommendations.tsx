...
export function RecommendationsPage(props: { revision: number; onFavouriteChanged: () => void; onHistoryChanged: () => void; onDiscover: () => void }) {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const next = await loadLocalRecommendations()
  ...
  return <ZStack frame={{ maxWidth: "infinity", maxHeight: "infinity" }}><PageBackground /><ScrollView navigationTitle="为你推荐" navigationBarTitleDisplayMode="large" refreshable={load} navigationDestination={{ isPresented: detailPresented, content: selected ? <DetailPage video={selected} onFavouriteChanged={props.onFavouriteChanged} onHistoryChanged={props.onHistoryChanged} /> : <VStack /> }}><VStack spacing={16} alignment="leading" padding={{ horizontal: PAGE_PADDING, top: 8, bottom: PAGE_BOTTOM_PADDING }}>
    ...
    {loading && items.length === 0 ? <StateView title="正在准备推荐" description="正在根据你的收藏和观看记录生成推荐。" loading /> : items.length === 0 && error ? <StateView title="暂时无法加载推荐" description={error} kind="error" action={() => { void load() }} /> : items.length === 0 ? <StateView title="暂无足够数据生成推荐" description="浏览、收藏或播放部分作品后，推荐内容将显示在这里。" systemImage="sparkles" action={props.onDiscover} actionTitle="前往浏览" /> : <VideoRowList items={items.map(item => item.video)} status={video => reasons.get(video.videoCode)} statusSystemImage="sparkles" onOpen={open} />}
    {error && items.length ? <StateView title="暂时无法更新" description="正在显示上次的推荐内容。" kind="error" action={() => { void load() }} actionTitle="重试" presentation="row" /> : undefined}
