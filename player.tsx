...
import { recordMissAVPlayback } from "./storage"
...
export async function chooseAndPresentMissAVPlayer(video: MissAVVideoItem, selected: MissAVVideoSource): Promise<MissAVPlaybackResult> {
  ...
  try {
    ...
    await presentNativeOnlinePlayer({ url: freshSource.url, headers: missavClient.playbackHeaders(freshDetail.watchUrl, freshSource.url), title: freshDetail.title, providerLabel: "MISSAV", qualityLabel: freshSource.label })
    return { opened: true }
  } catch (reason) {
